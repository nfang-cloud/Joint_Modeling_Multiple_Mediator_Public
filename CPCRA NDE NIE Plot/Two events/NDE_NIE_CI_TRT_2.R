...
cov1=read.table("covI_OL.txt",row.names=1)
...
est1=c(est$logL_r1,est$logL_r2,est$logL_r3,est$logL_r4,est$logL_r5,
        ...
        est$etamL,est$etamO,est$delta1,est$log_varc)
 ...
 write.csv(Bres1_new,paste("Bres_Multi_trt",as.character(1111+simseed),"batch=",as.character(batch),".csv",sep=""),row.names=FALSE)
         }
 }
